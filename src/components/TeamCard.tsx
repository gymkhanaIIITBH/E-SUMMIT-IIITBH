...
function TeamCard({name='Test', image=image1, designation='Marketing & PR Lead', email='testEMail', phone='Test Phone'}) {
  ...
  return (
    <>
<div className="overflow-hidden group mt-4 mb-4 relative w-64 h-auto flex flex-col justify-between shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out ">
  <img
    src={image}
    className="object-cover bg-black rounded-bl-3xl rounded-tr-3xl rounded-tl-sm rounded-br-sm w-full h-60 group-hover:opacity-90 mb-2 transition-transform duration-300 ease-in-out hover:scale-105"
    alt={name}
  />
  <div className="absolute inset-0 flex items-center justify-center  opacity-0 group-hover:opacity-100 transition-opacity duration-700 ease-in-out z-10">
    ...
    <span onClick={()=> handleCopy(phone)} className="text-3xl mx-4  font-bold text-neutral-50">
      ...
      {copied && <div className='block font-bold text-white absolute top-[60%] z-10'>Copied!</div>}
  </div>

  <div className="text-center rounded-b-lg font-newsreader">
    <div className="text-3xl leading-tight text-wrap font-bold text-neutral-800 mt-0">{name}</div>
    <div className="text-lg font-semibold texxt-wrap text-neutral-800/80">{designation}</div>
  </div>
</div>
