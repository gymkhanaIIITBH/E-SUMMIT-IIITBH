...
function TeamCard({name='Test', image=image1, designation='Marketing & PR Lead', email='', phone=null, linkedIn= ''}: {name: string, image: string, designation: string, email: string, phone: string | null, linkedIn: string}) {
...
<div className="overflow-hidden group mt-4 mb-4 relative w-64 h-auto flex flex-col  shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out bg-white text-black rounded-bl-3xl rounded-tr-3xl">
  <img
    ...
    alt={name}
  />
  <div className="absolute bg-opacity-30 bg-black inset-0 flex items-center justify-center  opacity-0 group-hover:opacity-100 transition-opacity duration-700 ease-in-out z-10">
 ...
 {phone && <span onClick={()=> handleCopy(phone)} className="text-3xl mx-2  font-bold text-black">
